...
%%%% This script reads the data file from OGCIO and outputs the figures
%%%% shown in the paper

%%%% Time 0 is 2021-07-16 00:00:01
%%%% Time unit is no. of minutes since time 0

load('Results/data_final');

%%%% "linelist{lang}" contains the data for langauge "lang" with the
%%%% following fields
%%%% 1) ip_idx: IP address (masked)
%%%% 2) time_visit: timestamps of vaccine messages delivered to that IP
%%%% 3) category: categories of vaccine messages delivered to that IP
%%%% 4) language: language of vaccine messages delivered to that IP
%%%% 5) booking_time: timestamps of vax bookings made from that IP
%%%% 6) booking_age: age of vaccinee for vax bookings made from that IP
...
for washout_period = [1]*day_to_min  % washout period could be 0.5, 1 and 2 days
    for margin_for_merging = [3]     % page requests for the same message from the same IP within this margin are regarded as a single page request
        clear processed_linelist
        for lang=1:numel(linelist_master)
            ...
            ip_lang_diff = [];
            ip_clash = [];
            linelist = linelist_master{lang};
            for ii=1:numel(linelist)
                disp([language_text{lang}  ': IP_idx ' num2str(ii)]);
                xx = linelist(ii);
                session_idx = 1;
                ...
                if (numel(xx.time_visit)>1)
                    ...
                    for iii=1:2
                        if (iii==1)
                            ...
                            fI = find(xx.time_visit_diff<washout_period);
                            if (~isempty(fI))
                                fI = unique([fI fI+1]);
                            end
                        end
                    ...
                    for jj=2:numel(xx.time_visit)
                        if (xx.time_visit_diff(jj-1)>washout_period)
            ...
            for ds_idx=ds_idx_vec
                ...
                for ii=1:numel(linelist_m)
                    ...
                    for session_idx=1:nn
                        if (numel(xx.time_visit_split{session_idx})==1)
                            ...
                            fI = find(abs(bk_time)<washout_period);
                            mm = numel(fI);
        ...
        %             for time_window_this = [180 360 720]         % follow-up period could be 3, 6 or 12 hours
        for time_window_this = 180         % follow-up period could be 3, 6 or 12 hours
            close all;
            ...
            for ds_idx=ds_idx_vec
                ...
                suffix = ['_lang' num2str(numel(linelist_master))...
                    '_washout' num2str(washout_period) ...
                    '_merge' num2str(margin_for_merging) ...
                    '_tw' num2str(time_window_this) ...
                    '_ds' num2str(ds_idx) ...
                    '_tp' num2str(numel(time_partition))];
                disp('');
                disp('Effectiveness that are significant:');
                fig_num = 0;
