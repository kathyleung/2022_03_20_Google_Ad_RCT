...
AA = readtable('Data/rev_data_matlab_no_merging');    %%%% Data pre-processed by Kathy
...
fI = find(AA.time_visit_rev > time_start & AA.time_visit_rev < time_end);
fI = fI';
tic
for ii=fI
...
linelist_large{1} = linelist;

save('Results/script_2_output');

for ii=1:numel(linelist)
    linelist(ii).ip = [];
    linelist(ii).booking_id = [];
    linelist(ii).vax_time = [];
end
clear booking_data AA XX master_data fI this_ip prev_ip this_booking_id
save('Results/script_2_output_masked');
